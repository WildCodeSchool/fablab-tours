...
purge() {
  local array=$(curl -v -u  ${SFTP_USER}:${SFTP_PASSWORD} ftp://funlab.fr/api.funlab.fr/public/$1);
  ...
  while read -r line; do
      ...
      then
        ...
        curl -u ${SFTP_USER}:${SFTP_PASSWORD} ftp://funlab.fr/api.funlab.fr/public/ -Q "RMD api.funlab.fr/public/$1$file/;quit"
      elif [[ $type = -* ]]
      then
        echo "Remove file ftp://funlab.fr/api.funlab.fr/public/$1$file"
        curl -u ${SFTP_USER}:${SFTP_PASSWORD} ftp://funlab.fr/api.funlab.fr/public/ -Q "DELE api.funlab.fr/public/$1$file;quit"
      fi
